...
public class DetallePagos implements IReportsCMCustomSearch {
	...
	public List<Properties> doExecute(CEService ceService, PEService peService, String objectStoreName, String rosterName, String eventLogName, Map<String, Object> condiciones) throws Exception {
		...
		if (!CommonUtils.isEmpty(condiciones.get("Cliente")))
			whereStatement.append("This INSUBFOLDER '/Facturas/" + condiciones.get("Cliente").toString() + "' AND isCurrentVersion = TRUE");		
		else
			whereStatement.append("This INSUBFOLDER '/Facturas' AND isCurrentVersion = TRUE");			
		...
		if (!CommonUtils.isEmpty(condiciones.get("FechaPagoHasta")))
			whereStatement.append(" AND FechaPago <= " + DateUtils.convertLocalTimeToUTC(longDateFormat.parse(condiciones.get("FechaPagoHasta").toString() + " 00:00:00")));	
		...
		for (Iterator<Document> it = docSet.iterator(); it.hasNext(); ) 
		{
			...
			props.setProperty("FechaPago", (pagoProps.getObjectValue("FechaPago") == null ? "" : shortDateFormat.format(pagoProps.getDateTimeValue("FechaPago"))));
