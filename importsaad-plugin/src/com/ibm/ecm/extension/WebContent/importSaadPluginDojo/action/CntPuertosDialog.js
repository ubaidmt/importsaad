define([
		...
		"dojo/text!./templates/CntPuertosDialog.html"
	],
	function(declare, 
			lang, 
			on, 
			...
			template) {
	/**
	 * @name importSaadPluginDojo.action.CntPuertosDialog
	 * @class Provides a dialog whose main content is an html page displayed in an iframe.  
	 * @augments ecm.widget.dialog.BaseDialog
	 */
	return declare("importSaadPluginDojo.action.CntPuertosDialog", [ BaseDialog ], {
	/** @lends importSaadPluginDojo.action.CntPuertosDialog.prototype */	
		...
		postCreate: function() {
			...
			this.setTitle("Administración de Puertos");
		...
		initGrid: function() {
			...
			var gridLayout = [
				{name: 'Puerto', field: 'name', width: '85%', editable: false, noresize: true},
		...
		searchPuertos: function(criterio, maxResults, callback) {			
		...
		loadPuertos: function() {
			...
			html.set(this.message, "Consultando puertos...");
			domStyle.set(this.progressBar, "display", "block");
			
			var criterio = {};
			this.searchPuertos(criterio, 0, lang.hitch(this, function(puertos) {  			
				...
				if (puertos.length == 1) { // select row if single result
		...
		onUpdate: function() {	
			...
			if (items.length <= 0) {
				domStyle.set(this.message, "color", "#6B0A0A");
				html.set(this.message, "Es necesario seleccionar un puerto.");
				domStyle.set(this.progressBar, "display", "none");			
				return;				
			}
			...
			// valida que el puerto a actualizar no exista previamente creado
			var criterio = {};
			criterio.name = this.name.getValue();
			this.searchPuertos(criterio, 1, lang.hitch(this, function(puertos) {  				
				
				if (puertos.length > 0 && puertos[0].id.toString() != item.id.toString()) {
					domStyle.set(this.message, "color", "#6B0A0A");
					html.set(this.message, "El puerto " + this.name.getValue() + " ya existe.");
					domStyle.set(this.progressBar, "display", "none");				
					return;	
				}
				
				domStyle.set(this.message, "color", "#000253");
				html.set(this.message, "Actualizando el puerto seleccionado...");
				domStyle.set(this.progressBar, "display", "block");
				
				var params = {};
				params.method = "updatePuerto";
				params.repositoryid = ecm.model.desktop.defaultRepositoryId;
				params.context = json.stringify(this.config.context);
				var puerto = {"id": item.id.toString(), "name": this.name.getValue(), "activo": this.activo.getValue().toString() === "true"};
				params.puerto = json.stringify(puerto);
				...
				Request.invokePluginService("ImportSaadPlugin", "CntCatalogosService",
						{	
							...
							requestCompleteCallback: lang.hitch(this, function(response) {
								...
								// Update grid store						
								this.grid.store.setValue(item, 'name', response.puerto.name);
								this.grid.store.setValue(item, 'activo', response.puerto.activo);
								
								domStyle.set(this.message, "color", "#000253");
								html.set(this.message, "El puerto " + response.puerto.name + " ha sido actualizado.");
								domStyle.set(this.progressBar, "display", "none");
		...
		onDelete: function() {
			...
			if (items.length <= 0) {
				domStyle.set(this.message, "color", "#6B0A0A");
				html.set(this.message, "Es necesario seleccionar por lo menos un puerto.");
				domStyle.set(this.progressBar, "display", "none");			
				return;				
			}
						
			var confirmDialog = new YesNoCancelDialog({
				title: "Eliminar Puertos",
				text: "¿Está seguro de eliminar los <b>" + items.length + "</b> puertos seleccionados?<br><br>Serán eliminados únicamente los puertos que no tengan contenedores asociados.",
				onYes: lang.hitch(this, function() {
					confirmDialog.hide();
					
					domStyle.set(this.message, "color", "#000253");
					html.set(this.message, "Eliminando puertos seleccionados...");
					domStyle.set(this.progressBar, "display", "block");
					
					var puertos = [];
					array.forEach(items, lang.hitch(this, function(item) {							
						var puerto = {"id": item.id.toString()};
						puertos.push(puerto);
					}));					

					var params = {};
					params.method = "deletePuerto";
					params.repositoryid = ecm.model.desktop.defaultRepositoryId;
					params.context = json.stringify(this.config.context);
					params.puertos = json.stringify(puertos);
					...
					Request.invokePluginService("ImportSaadPlugin", "CntCatalogosService",
							{	
								...
								requestCompleteCallback: lang.hitch(this, function(response) {
									...
									domStyle.set(this.message, "color", "#000253");
									html.set(this.message, "Se eliminaron " + response.count + " de los " + puertos.length + " puertos seleccionados.");
									domStyle.set(this.progressBar, "display", "none");									
		...
		onAdd: function() {	
			...
			// valida que la puerto no exista previamente creada
			var criterio = {};
			criterio.name = this.name.getValue();
			this.searchPuertos(criterio, 1, lang.hitch(this, function(puertos) {  
				...
				domStyle.set(this.message, "color", "#000253");
				html.set(this.message, "Agregando nuevo puerto...");
				domStyle.set(this.progressBar, "display", "block");	
				
				var params = {};
				params.method = "addPuerto";
				params.repositoryid = ecm.model.desktop.defaultRepositoryId;
				params.context = json.stringify(this.config.context);
				var puerto = {"name": this.name.getValue(), "activo": this.activo.getValue().toString() === "true"};
				params.puerto = json.stringify(puerto);
				...
				Request.invokePluginService("ImportSaadPlugin", "CntCatalogosService",
						{	
							...
							requestCompleteCallback: lang.hitch(this, function(response) {
								...
								this.grid.store.newItem(response.puerto);
								this.grid.store.save();
								
								domStyle.set(this.message, "color", "#000253");
								html.set(this.message, "El nuevo puerto " + response.puerto.name + " ha sido creado.");
								domStyle.set(this.progressBar, "display", "none");
